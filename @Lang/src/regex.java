...
public class regex {
    ...
    private static final String ATOMIC_EXPR = "\\(*(" + INT + "|" + DEC + "|" + VAR + ")\\)*";  

    // Operators with Precedence
    private static final String EXP = ATOMIC_EXPR + "(\\s*\\^\\s*" + ATOMIC_EXPR + ")*";
    private static final String MUL_DIV_MOD = EXP + "(\\s*[*/%]\\s*" + EXP + ")*";
    private static final String ADD_SUB = MUL_DIV_MOD + "(\\s*[+-]\\s*" + MUL_DIV_MOD + ")*";

    // Assignment with Type Constraints
    private static final String ASSIGN_REGEX =
        "^(" + VAR + "@INT\\s*@\\s*" + INT + "|" +
              VAR + "@DEC\\s*@\\s*" + DEC + "|" +
              VAR + "@BOOL\\s*@\\s*" + BOOL + "|" +
              VAR + "@CHAR\\s*@\\s*" + CHAR + "|" +
    ...
    private static final Pattern SCOPE = Pattern.compile("^\\{\\s*(" + ASSIGN_REGEX + "|.*\\n?)*\\s*}$", Pattern.MULTILINE);
    ...
    public static void main(String[] args) {
        ...
        // Invalid cases
        System.out.println("ASSIGN Validation (invalid, should be false: x@INT @ 5.5 + 3;): " + validate("x@INT @ 5.5 + 3;", ASSIGN));
